...
````
2. find the longest string matching "*/" from first in $0
```sh
${0##*/}
````
3. find the longest string matching "b" from last in $string
```sh
${str%%b}
```
4. Find email addresses in a file
```sh
grep -E -o "\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,6}\b" file.txt
```
5. Print 5th to 10th line in file
```sh
sed -n '5,10p' file                    # using sed
awk 'NR>=5{print} NR==10{exit}' file   # using awk
```
6. Extract lines between two patterns
```sh
sed -n '/pattern1/,/pattern2/p' file
```
7. Remove dups but preserve the order
```sh
awk '!visited[$0]++' your_file > deduplicated_file
```
8. Copy from bash to clipboard using OSC command
```
echo -e "\033]52;c;$(base64 <<< "copy something here")\a"
```
9. Raise notification from terminal using OSC command
```
echo -e "\033]9;This is a notification\a"
```

10. Cartesian product in glob
```
echo repos/ntrivedi/code/{0,1}{1..9}.cpp
```
